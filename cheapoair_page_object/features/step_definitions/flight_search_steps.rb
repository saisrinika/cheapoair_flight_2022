...
Then(/^verify user should see the avialable flights$/) do
  pending
end
...
Given(/^user is on the cheapoair homrpage$/) do
  pending
end

And(/^user choose a roundtrip option$/) do
  pending
end

And(/^user choose future dates for arrival and departure dates$/) do
  pending
end

And(/^search for the availbilty flights$/) do
  pending
end

Then(/^user should see the "([^"]*)"$/) do |arg, table|
  # table is a table.hashes.keys # => [:Please enter a From city or airport.]
  pending
end